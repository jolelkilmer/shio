...
public class ShPostUtils {
  ...
  @Autowired private ShPostDraftRepository shPostDraftRepository;
  @Autowired private ShPostAttrRepository shPostAttrRepository;
  @Autowired private ShPostDraftAttrRepository shPostDraftAttrRepository;
  ...
  @Autowired private ShReferenceDraftRepository shReferenceDraftRepository;
  @Autowired private ShRelatorItemRepository shRelatorItemRepository;
  ...
  public ShPost getShPostFromObjectId(String objectId) {
    ...
    if (shPostOpt.isPresent()) return shPostOpt.get();
    else return null;
  }

  public ShPost copy(ShPostImpl shPost, ShFolder shFolderDest) {

    List<ShPostTinyBean> shPostTinyBeans =
        shPostRepository.findByShFolderTiny(shFolderDest.getId());
  ...
  private void createNewReference(ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    try {
      ShObject shObjectReferenced =
          shObjectRepository.findById(shPostAttr.getStrValue()).orElse(null);
  ...
  public void referencedObjectDraft(ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    if (shPostAttr != null
        && shPostAttr.getShPostTypeAttr() != null
      ...
      if (shPostAttr.getShPostTypeAttr().getShWidget().getName().equals(ShSystemWidget.FILE)) {
        this.referencedFileDraft(shPostAttr, shPost);
          ...
          .getShWidget()
          .getName()
          .equals(ShSystemWidget.CONTENT_SELECT)) {
  ...
  public void referencedObject(
      ShPostAttrImpl shPostAttrEdit, ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    if (shPostAttr != null
        && shPostAttr.getShPostTypeAttr() != null
        && shPostAttr.getShPostTypeAttr().getShWidget() != null) {
      if (shPostAttr.getShPostTypeAttr().getShWidget().getName().equals(ShSystemWidget.FILE)) {
  ...
  public void referencedFile(
      ShPostAttrImpl shPostAttrEdit, ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    if (shPost != null
        && shPost.getShPostType() != null
        && shPost.getShPostType().getName() != null
        && shPost.getShPostType().getName().equals(ShSystemPostType.FILE)) {
      File fileFrom =
          shStaticFileUtils.filePath(shPost.getShFolder(), shPostAttrEdit.getStrValue());
  ...
  private void removeOldReferenceDraft(ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    ...
    // a valid reference
    // Remove old references
    List<ShReferenceDraft> shOldReferences =
        shReferenceDraftRepository.findByShObjectFrom((ShObjectDraft) shPost);
    if (!shOldReferences.isEmpty()) {
  ...
  public void updateRelatorInfo(ShPostAttrImpl shPostAttr, ShPostImpl shPost) {
    for (ShRelatorItemImpl shRelatorItem : shPostAttr.getShChildrenRelatorItems()) {
      ...
      for (ShPostAttrImpl shChildrenPostAttr :
          postAttrsSort((Set<ShPostAttrImpl>) shRelatorItem.getShChildrenPostAttrs())) {
        ...
        this.setPostSummary(
            shPostAttr, shRelatorItem, title, summary, shChildrenPostAttr, shPostTypeAttr);
  ...
  private void setPostSummary(
      ShPostAttrImpl shPostAttr,
      ShRelatorItemImpl shRelatorItem,
      StringBuilder title,
      StringBuilder summary,
      ShPostAttrImpl shChildrenPostAttr,
    ...
    if (shPostTypeAttr.getIsSummary() == 1) {
      ...
      if (isReferencedWidget(shChildrenPostAttr, widgetName)) {
        ...
        if (!StringUtils.isEmpty(title.toString())) title.append(", ");
  ...
  private boolean isReferencedWidget(ShPostAttrImpl shChildrenPostAttr, String widgetName) {
    return shChildrenPostAttr.getReferenceObject() != null
  ...
  private void summaryDate(ShPostAttrImpl shPostAttr, StringBuilder title, StringBuilder summary) {
    ...
    if (!StringUtils.isEmpty(title.toString())) summary.append(", ");
    summary.append(dt.format(shPostAttr.getDateValue()));
  }

  private void summaryRelator(
      StringBuilder title, StringBuilder summary, ShPostAttrImpl shChildrenPostAttr) {
  ...
  private StringBuilder setPostTitle(
      ShRelatorItemImpl shRelatorItem,
      StringBuilder title,
      ShPostAttrImpl shChildrenPostAttr,
      ShPostTypeAttr shPostTypeAttr) {
  ...
  public List<ShPostAttrImpl> postAttrsSort(Set<ShPostAttrImpl> shPostAttrs) {
    ...
    Collections.sort(
        shPostAttrsByOrdinal,
        (ShPostAttrImpl o1, ShPostAttrImpl o2) ->
            o1.getShPostTypeAttr().getOrdinal() - o2.getShPostTypeAttr().getOrdinal());
  ...
  public ShPost loadLazyPost(String id, boolean getPostPublished) {
    ...
    if (!shPostOptional.isPresent()) return null;
      ...
      try {
        ...
        String jsonInString =
            mapper
                ...
                .writerWithView(ShJsonView.ShJsonViewObject.class)
                .writeValueAsString(shPostDraft)
      ...
      try {
        String jsonInString =
            mapper
                ...
                .disable(SerializationFeature.FAIL_ON_EMPTY_BEANS)
                .writerWithView(ShJsonView.ShJsonViewObject.class)
                .writeValueAsString(shPostDraftAttr);
  ...
  private void loadPostAttribs(Set<ShPostAttr> shPostAttrs) {
    for (ShPostAttrImpl shPostAttr : shPostAttrs) {
      shPostAttr.setShChildrenRelatorItems(
          shRelatorItemRepository.findByShParentPostAttr(shPostAttr));
      shPostAttr
          .getShPostTypeAttr()
          .setShPostTypeAttrs(
      ...
      if (!shPostAttr.getShChildrenRelatorItems().isEmpty()) {
        shPostAttr
            .getShChildrenRelatorItems()
            .forEach(
                shRelatorItem -> {
                  shRelatorItem.setShChildrenPostAttrs(
                      shPostAttrRepository.findByShParentRelatorItemJoin(shRelatorItem));
  ...
  private Map<String, ShPostTypeAttr> postTypeAttrMap(ShPostTypeAttr shPostTypeAttr) {
    Map<String, ShPostTypeAttr> shPostTypeAttrMap = new HashMap<>();
    Set<ShPostTypeAttr> shPostTypeAttrs =
        shPostTypeAttrRepository.findByShParentPostTypeAttr(shPostTypeAttr);
    ...
    if (shPost != null) {
      shPost
          ...
          .forEach(
              shPostAttr ->
                  shPostAttrMap.put(
                      ((ShPostAttr) shPostAttr).getShPostTypeAttr().getId(),
  ...
  private Map<String, ShPostAttr> postAttrMap(ShRelatorItemImpl shRelatorItem) {
    ...
    if (shRelatorItem.getShChildrenPostAttrs() != null) {
      shRelatorItem
          .getShChildrenPostAttrs()
          .forEach(
              shPostAttr ->
                  shPostAttrMap.put(
                      shPostAttr.getShPostTypeAttr().getId(), (ShPostAttr) shPostAttr));
    }
    return shPostAttrMap;
  }
   ...
   */
  private void postAttrNotInPostType(
      Set<ShPostAttr> shPostAttrs,
      Map<String, ShPostAttr> shPostAttrMap,
  ...
  private void postAttrNotInPostTypeNested(ShPostAttrImpl shPostAttr) {
    if (shPostAttr.getShChildrenRelatorItems() != null) {
      for (ShRelatorItemImpl shRelatorItem : shPostAttr.getShChildrenRelatorItems()) {
        if (shRelatorItem.getShChildrenPostAttrs() != null) {
          Map<String, ShPostTypeAttr> shPostTypeAttrChildMap =
              this.postTypeAttrMap(shPostAttr.getShPostTypeAttr());
   ...
   * @param shPostTypeAttrMap PostTypeAttrMap Object
   */
  private void postAttrInPostType(
      Set<ShPostAttr> shPostAttrs,
      Map<String, ShPostAttr> shPostAttrMap,
      Map<String, ShPostTypeAttr> shPostTypeAttrMap) {
  ...
  public boolean allowPublish(ShPostType shPostType, Principal principal) {
    if (principal == null
        || (shPostType != null && StringUtils.isEmpty(shPostType.getWorkflowPublishEntity()))) {
      return true;
    } else {
  ...
  private boolean isPublished(ShPostType shPostType, List<ShUser> shUsers) {
    return shPostType != null
        && !StringUtils.isEmpty(shPostType.getWorkflowPublishEntity())
        && shGroupRepository.countByNameAndShUsersIn(shPostType.getWorkflowPublishEntity(), shUsers)
            > 0;
  }
}
