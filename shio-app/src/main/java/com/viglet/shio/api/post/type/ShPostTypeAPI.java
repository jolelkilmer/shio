...
public class ShPostTypeAPI {
  ...
  public ShPostType shPostTypeEdit(@PathVariable String id) {
    ...
    if (shPostType.isPresent()) {
      shPostType
          .get()
          .setShPostTypeAttrs(shPostTypeAttrRepository.findByShPostType(shPostType.get()));
      ...
      shPostType
          ...
          .forEach(
              shPostTypeAttr -> {
                shPostTypeAttr
                    .getShPostAttrs()
                    .forEach(shPostAttr -> shPostAttrRepository.delete(shPostAttr.getId()));
  ...
  @PostMapping("/{id}/attr")
  @JsonView({ShJsonView.ShJsonViewPostType.class})
  public ShPostTypeAttr shPostTypeAttrAdd(
      @PathVariable String id, @RequestBody ShPostTypeAttr shPostTypeAttr) {
    Optional<ShPostType> shPostTypeOptional = shPostTypeRepository.findById(id);
    if (shPostTypeOptional.isPresent()) {
      ShPostType shPostType = shPostTypeOptional.get();
      if (shPostType != null) {
