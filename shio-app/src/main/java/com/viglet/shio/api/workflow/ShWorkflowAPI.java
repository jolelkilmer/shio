...
public class ShWorkflowAPI {
  ...
  public Set<ShWorkflowTask> shWorkflowTasksGet(Principal principal) {
    ...
    shGroupRepository
        .findByShUsersIn(shUsers)
        .forEach(shGroup -> requesters.add(shGroup.getName()));

    Set<ShWorkflowTask> shWorkflowTasks = shWorkflowTaskRepository.findByRequestedIn(requesters);

    shWorkflowTasks.forEach(
        shWorkflowTask ->
            shPostRepository
                .findByIdFull(shWorkflowTask.getShObject().getId())
                .ifPresent(shWorkflowTask::setShObject));
