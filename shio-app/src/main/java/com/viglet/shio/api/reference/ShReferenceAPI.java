...
public class ShReferenceAPI {
  ...
  public List<ShReference> shReferenceFrom(@PathVariable String fromId) {
    ...
    if (shObject.isPresent()) return shReferenceRepository.findByShObjectFrom(shObject.get());
  ...
  public List<ShReference> shReferenceTo(@PathVariable String toId) {
    ...
    if (shObject.isPresent()) return shReferenceRepository.findByShObjectTo(shObject.get());
    return Collections.emptyList();
  }

  @PostMapping("/to/{toId}/replace/{otherId}")
  @JsonView({ShJsonView.ShJsonViewReference.class})
  public List<ShReference> shReferenceToReplace(
      @PathVariable String toId, @PathVariable String otherId) {
    Optional<ShObject> shObject = shObjectRepository.findById(toId);
  ...
  private void setReferenceToPostAttr(
      ShObjectImpl shObject, ShObject shObjectOther, ShReference shReference) {
    if (shReference.getShObjectFrom() instanceof ShPost) {
      ShPostImpl shPost = (ShPostImpl) shReference.getShObjectFrom();
      shPost
          .getShPostAttrsNonDraft()
          .forEach(
              shPostAttr -> {
