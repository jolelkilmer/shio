...
public class ShHistoryAPI {
  ...
  @Autowired private ShHistoryPageableRepository shHistoryPageableRepository;
    ...
    if (shObjectRepository.findById(globalId).isPresent()) {
      ...
      if (shObject != null) {
        if (shObject instanceof ShSite shSite) {
          ...
          return shHistoryPageableRepository.findByShObjectOrderByDateDesc(
              shObject.getId(), pageable);
        }
      }
    }
