...
public class ShUserAPI {
  ...
  private boolean isAdminFromNative(ShUser shUser, ShAuthProviderInstance instance) {
    ...
    if (!isNotNativeAuth(instance) && shUser != null && shUser.getShGroups() != null) {
      for (ShGroup shGroup : shUser.getShGroups()) {
        if (shGroup.getName().equals("Administrator")) isAdmin = true;
      }
    }
    return isAdmin;
  }
  ...
  private boolean isNotNativeAuth(ShAuthProviderInstance instance) {
    return instance != null
        && !instance.getVendor().getId().equals(ShAuthSystemProviderVendor.NATIVE);
  }
  ...
  private ShUser loadAuthProvider(
      String currentUserName, ShAuthProviderInstance instance, ShUser shUser) {
    ShAuthenticationProvider shAuthenticationProvider;
    try {
      shAuthenticationProvider =
          (ShAuthenticationProvider)
              context.getBean(Class.forName(instance.getVendor().getClassName()));
