...
public class ShPostAPI {
  ...
  @Autowired private ShHistoryUtils shHistoryUtils;
  ...
  @PutMapping("/{id}")
  @JsonView({ShJsonView.ShJsonViewObject.class})
  public ResponseEntity<ShPost> shPostUpdate(
      @PathVariable String id, @RequestBody ShPost shPost, Principal principal) {
    if (shObjectUtils.canAccess(principal, id)) {
  ...
  public ResponseEntity<Boolean> shPostDelete(@PathVariable String id, Principal principal) {
    if (shObjectUtils.canAccess(principal, id)) {
      ...
      if (shPostOptional.isPresent()) {
        ...
        shReferenceDraftRepository.deleteAllInBatch(
            shReferenceDraftRepository.findByShObjectTo(shPost));
  ...
  @JsonView({ShJsonView.ShJsonViewObject.class})
  public ResponseEntity<StreamingResponseBody> shPostExport(
  ...
  private void deleteStaticFiles(ShPost shPost) {
    ...
    if (shPost.getShPostType().getName().equals(ShSystemPostType.FILE) && !shPostAttrs.isEmpty()) {
      File file =
          shStaticFileUtils.filePath(
              shPost.getShFolder(), shPostAttrs.iterator().next().getStrValue());
  ...
  public void postSave(ShPost shPost) {
    ...
    if (shPost.getPublishStatus() != null) {
      ...
      if (shPost.isPublished()) this.postDraftSave(shPost);
      else this.postUnpublishSave(shPost);
    }
  ...
  private void titleAndSummary(
      StringBuilder title, StringBuilder summary, List<ShPostAttr> shPostAttrsByOrdinal) {
    shPostAttrsByOrdinal.forEach(
        shPostAttr -> {
          this.setTitle(title, shPostAttr);
          this.setSummary(summary, shPostAttr);
          if (shPostAttr != null) shPostAttr.setReferenceObject(null);
        });
  }

  private void setSummary(StringBuilder summary, ShPostAttr shPostAttr) {
    if (shPostAttr.getShPostTypeAttr().getIsSummary() == 1) {
      if (!StringUtils.isEmpty(summary.toString())) summary.append(", ");
  ...
  private void postDraftDelete(String id) {
    ...
    if (shPostOptional.isPresent()) {
      ...
      shReferenceDraftRepository.deleteAllInBatch(
          shReferenceDraftRepository.findByShObjectFrom(shPost));
  ...
  public void postDraftSave(ShPost shPost) {
    if (shPost.getId() != null && shPost.isPublished()) {
      shPostRepository
          .findById(shPost.getId())
          .ifPresent(
              shPostEdit -> {
                ShPostDraft shPostDraft = convertPost2Draft(shPost);
                @SuppressWarnings("unchecked")
                Set<ShPostDraftAttr> shPostAttrs =
                    (Set<ShPostDraftAttr>) shPostDraft.getShPostAttrs();
                shPostAttrs.forEach(
                    shPostAttr -> {
  ...
  private void postReferenceSave(ShPostImpl shPostImpl) {
    ...
    if (shPostImpl instanceof ShPost shPost) {
      ...
      List<ShReferenceDraft> shOldReferences =
          shReferenceDraftRepository.findByShObjectFrom(shPostDraft);
    ...
    shPostImpl
        .getShPostAttrs()
        .forEach(shPostAttr -> shPostUtils.updateRelatorInfo(shPostAttr, shPostImpl));
