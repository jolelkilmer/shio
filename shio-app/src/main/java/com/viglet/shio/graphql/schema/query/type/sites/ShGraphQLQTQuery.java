...
public class ShGraphQLQTQuery {
    ...
    queryTypeBuilder.field(
        newFieldDefinition()
            ...
            .argument(
                newArgument()
                    .name(POST_ATTR_NAME)
                    .description("Post Attribute Name")
                    .type(GraphQLString))
            .argument(
                newArgument()
                    .name(ARRAY_VALUE)
  ...
  private DataFetcher<List<Map<String, Object>>> getDataFetcher() {
    return dataFetchingEnvironment -> {
      ...
      if (folderId == null && postAttrName == null && arrayValue == null) {
        shQueryComponent
            .findByPostTypeName(postTypeName)
            .forEach(post -> results.add(postToGraphQL(post)));
      } else if (folderId != null) {
        shQueryComponent
            .findByFolderName(folderId, postTypeName)
            .forEach(post -> results.add(postToGraphQL(post)));
      } else if (postAttrName != null && arrayValue != null) {
        shQueryComponent
            .findByPostTypeNameIn(postTypeName, Sets.newHashSet(arrayValue))
            .forEach(post -> results.add(postToGraphQL(post)));
      }
  ...
  private Map<String, Object> postToGraphQL(Map<String, ShPostAttr> post) {
    ...
    try {
      jsonInString =
          mapper.writerWithView(ShJsonView.ShJsonViewObject.class).writeValueAsString(post);

      Map<String, ShPostAttr> tinyPost =
          mapper.readValue(jsonInString, new TypeReference<Map<String, ShPostAttr>>() {});
