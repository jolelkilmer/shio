...
public class ShGraphQLQTPlural {
  ...
  public void createQueryTypePlural(
    ...
    if (!StringUtils.isBlank(shPostType.getName())
      ...
      shGraphQLQTCommons.createArguments(
          queryTypeBuilder, graphQLObjectType, postTypeNamePlural, postTypeWhereInputBuilder, true);

      codeRegistryBuilder.dataFetcher(
          coordinates(ShGraphQLConstants.QUERY_TYPE, postTypeNamePlural),
          this.getPostTypeAllDataFetcherPlural(shPostType));
    }
  }

  private void whereFieldsPlural(
      ShPostType shPostType, GraphQLInputObjectType.Builder postTypeWhereInputBuilder) {

    String whereInputName =
        shPostType.getName().replace("-", "_").concat(ShGraphQLConstants.WHERE_INPUT);

    GraphQLTypeReference whereInputRef = GraphQLTypeReference.typeRef(whereInputName);

    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.SEARCH,
        GraphQLString,
        "Contains search across all appropriate fields.");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.AND,
        whereInputRef,
        "Logical AND on all given filters.");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.OR,
        whereInputRef,
        "Logical OR on all given filters.");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.NOT,
        whereInputRef,
        "Logical NOT on all given filters combined by AND.");

    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.ID,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_ID,
        "Identifier");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.TITLE,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Title");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.DESCRIPTION,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Description");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.FURL,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Friendly URL");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.MODIFIER,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Modifier");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.PUBLISHER,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Publisher");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.FOLDER,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_STRING,
        "Folder Name");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.CREATED_AT,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_DATE_TIME,
        "Created Date");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.UPDATED_AT,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_DATE_TIME,
        "Updated Date");
    shGraphQLInputObjectField.createInputObjectField(
        postTypeWhereInputBuilder,
        ShGraphQLConstants.PUBLISHED_AT,
        ShGraphQLConstants.FIELD_TYPE_GRAPHQL_DATE_TIME,
        "Published Date");
  ...
  private DataFetcher<List<Map<String, String>>> getPostTypeAllDataFetcherPlural(
      ShPostType shPostType) {
    ...
    return dataFetchingEnvironment -> {
      ...
      Map<String, Object> whereMap =
          dataFetchingEnvironment.getArgument(ShGraphQLConstants.WHERE_ARG);

      if (whereMap != null) {
        this.whereCondition(shPostType, posts, siteIds, whereMap);
      } else {
        shPostRepository
            .findByShPostType(shPostType)
        ...
        .forEach(
            whereArgItem -> {
              ...
              if (arg.equals(ShGraphQLConstants.SEARCH)) {
                ...
                if (arg.startsWith("_")
                    && arg.replaceFirst("_", StringUtils.EMPTY)
                        .contains(ShGraphQLConstants.CONDITION_SEPARATOR)) {
                  field =
                      String.format("_%s", arg.split(ShGraphQLConstants.CONDITION_SEPARATOR)[1]);
                  action =
                      arg.replaceFirst(
                          field.concat(ShGraphQLConstants.CONDITION_SEPARATOR), StringUtils.EMPTY);

                } else if (!arg.startsWith("_")
                    && arg.contains(ShGraphQLConstants.CONDITION_SEPARATOR)) {
                  field = arg.split(ShGraphQLConstants.CONDITION_SEPARATOR)[0];
                  action =
                      arg.replaceFirst(
                          field.concat(ShGraphQLConstants.CONDITION_SEPARATOR), StringUtils.EMPTY);
                }
                shGraphQLInputObjectField.fieldWhereCondition(
                    shPostType, posts, whereArgItem, field, action, siteIds);
              }
            });
  }
}
