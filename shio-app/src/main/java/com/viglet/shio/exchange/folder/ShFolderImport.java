...
public class ShFolderImport {
  ...
  public void shFolderImportNested(
      String shObject,
      boolean importOnlyFolders,
      ShExchangeObjectMap shExchangeObjectMap,
      ShExchangeContext shExchangeContext) {
    ...
    if (shChildObjects.containsKey(shObject)) {
      for (String objectId : shChildObjects.get(shObject)) {
        ...
        if (!importOnlyFolders
            && shObjects.get(objectId) instanceof ShPostExchange shPostExchange) {
  ...
  public ShFolder createShFolder(ShFolderExchangeContext context) {
    ...
    Optional<ShFolder> shFolderOptional =
        shFolderRepository.findById(context.getShFolderExchange().getId());
    if (shFolderOptional.isPresent()) {
      shFolderChild = shFolderOptional.get();
    } else {
      shFolderChild =
          this.createFolderObject(
              context.getShFolderExchange(),
              context.getUsername(),
              context.getShObject(),
              context.getShExchangeObjectMap().getShObjects(),
              context.isCloned());
    }

    this.shFolderImportNested(
        shFolderChild.getId(),
        context.isImportOnlyFolders(),
  ...
  private ShFolder createFolderObject(
    ...
    logger.info(
        String.format("...... %s Folder (%s)", shFolderChild.getName(), shFolderChild.getId()));
    shFolderRepository.save(shFolderChild);
    return shFolderChild;
  }

  private void rootFolderSettings(
      ShFolderExchange shFolderExchange,
      String shObject,
      Map<String, Object> shObjects,
      ShFolder shFolderChild) {
    if (shFolderExchange.getParentFolder() != null) {
      ShFolder parentFolder =
          shFolderRepository.findById(shFolderExchange.getParentFolder()).orElse(null);
