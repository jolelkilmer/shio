...
public class ShExchangeFilesDirs {
  ...
  public void deleteExport() {
    ...
    try {
      if (this.parentExportDir != null) FileUtils.deleteDirectory(this.parentExportDir);
      else FileUtils.deleteDirectory(this.exportDir);
  ...
  public File updateExportJsonFilePath() {
    this.exportJsonFile =
        new File(this.exportDir.getAbsolutePath().concat(File.separator + EXPORT_FILE));
