...
public class ShPostExport {
    ...
    if (shPost.isPresent()) {
      ...
      if (shExchangeFilesDirs.generate()) {
        ...
        return shExchangeUtils.downloadZipFile(
            String.format("%s_post", shPost.get().getFurl()),
            response,
            shExchange,
            shExchangeFilesDirs);
      }
  ...
  public void shPostAttrExchangeIterate(
      ShPost shPost, Map<String, Object> fields, List<ShFileExchange> files) {
    shPost
        .getShPostAttrs()
        .forEach(
            shPostAttr -> {
  ...
  private void exportAttrs(Map<String, Object> fields, ShPostAttrImpl shPostAttr) {
    if (!shPostAttr.getArrayValue().isEmpty())
      ...
      fields.put(
          shPostAttr.getShPostTypeAttr().getName(), dateFormat.format(shPostAttr.getDateValue()));
    } else {
      fields.put(shPostAttr.getShPostTypeAttr().getName(), shPostAttr.getStrValue());
    }
  }

  private void exportRelatorAttrs(
      ShPost shPost,
  ...
  private boolean canExportStaticFile(
      ShPostImpl shPost, List<ShFileExchange> files, ShPostAttrImpl shPostAttr) {
    return files != null
        && shPostAttr.getShPostTypeAttr().getName().equals(ShSystemPostTypeAttr.FILE)
        && shPost.getShPostType().getName().equals(ShSystemPostType.FILE);
  }

  private void exportStaticFiles(
      ShPost shPost, List<ShFileExchange> files, ShPostAttrImpl shPostAttr) {
