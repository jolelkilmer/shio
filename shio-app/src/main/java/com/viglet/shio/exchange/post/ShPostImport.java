...
public class ShPostImport {
  ...
  public ShPostImpl getShPost(ShPostExchange shPostExchange) {
    ...
    for (Entry<String, Object> shPostField : shPostExchange.getFields().entrySet()) {
      ShPostTypeAttr shPostTypeAttr =
          shPostTypeAttrRepository.findByShPostTypeAndName(
              shPost.getShPostType(), shPostField.getKey());
  ...
  private void getShPostAttrs(
      ShPostExchange shPostExchange,
      ShPost shPost,
    ...
    for (Entry<String, Object> shPostField : shPostFields.entrySet()) {
      ...
      ShPostTypeAttr shPostTypeAttr =
          this.getPostAttr(shParentRelatorItem, shPostField, shPostType);

      if (isRelator(shPostTypeAttr)) {
        this.postAttrRelator(
            shPostExchange, shPost, shParentRelatorItem, shPostField, shPostTypeAttr);
      } else {
  ...
  private ShPostTypeAttr getPostAttr(
      ShRelatorItemImpl shParentRelatorItem,
      Entry<String, Object> shPostField,
      ShPostType shPostType) {
    ShPostTypeAttr shPostTypeAttr =
        shPostTypeAttrRepository.findByShPostTypeAndName(shPostType, shPostField.getKey());
    // Relator: the PostType is null
    if (shPostTypeAttr == null && shParentRelatorItem != null) {
      shPostTypeAttr =
          shPostTypeAttrRepository.findByShParentPostTypeAttrAndName(
  ...
  private boolean isRelator(ShPostTypeAttr shPostTypeAttr) {
    return shPostTypeAttr != null
        && shPostTypeAttr.getShWidget() != null
        && shPostTypeAttr.getShWidget().getName().equals(ShSystemWidget.RELATOR);
  }

  private void postAttrNonRelator(
      ShPost shPost,
      ShRelatorItemImpl shParentRelatorItem,
      Entry<String, Object> shPostField,
      ShPostTypeAttr shPostTypeAttr) {
  ...
  private ShPostAttr importPostAttr(
      ShPost shPost,
      ShRelatorItemImpl shParentRelatorItem,
      Entry<String, Object> shPostField,
      ShPostTypeAttr shPostTypeAttr) {
  ...
  private void setPostAttrValue(
      Entry<String, Object> shPostField, ShPostTypeAttr shPostTypeAttr, ShPostAttr shPostAttr) {
  ...
  private void setTittleAndSummaryFromRelator(
      ShRelatorItemImpl shParentRelatorItem, ShPostTypeAttr shPostTypeAttr, ShPostAttr shPostAttr) {
  ...
  @SuppressWarnings({"unchecked"})
  private void postAttrRelator(
      ShPostExchange shPostExchange,
      ShPost shPost,
      ShRelatorItemImpl shParentRelatorItem,
      Entry<String, Object> shPostField,
      ShPostTypeAttr shPostTypeAttr) {
    LinkedHashMap<String, Object> relatorFields =
        (LinkedHashMap<String, Object>) shPostField.getValue();
  ...
  public ShPostImpl createShPost(
      ShExchangeContext context,
      ShPostExchange shPostExchange,
    ...
    if (shPost != null) logger.info("........ {} Post ({})", shPost.getTitle(), shPost.getId());
    return shPost;
  }

  private ShPost extractPostFromExchange(
      ShExchangeContext context,
      ShPostExchange shPostExchange,
      ShExchangeObjectMap shExchangeObjectMap) {
    ShPost shPost;
    ...
    if (shPostExchange.getPosition() > 0) shPost.setPosition(shPostExchange.getPosition());
    ...
    if (shPostExchange.getOwner() != null) shPost.setOwner(shPostExchange.getOwner());
    else shPost.setOwner(shUserUtils.getCurrentUsername());
    ...
    this.createShPostAttrs(
        context, shPostExchange, shPost, shPostExchange.getFields(), null, shExchangeObjectMap);

    for (ShPostAttrImpl shPostAttr : shPostAttrRepository.findByShPost(shPost)) {
      shPostUtils.updateRelatorInfo(shPostAttr, shPost);
    }

    if (turingEnabled) shTuringIntegration.indexObject(shPost);
    return shPost;
  }

  private void detectPostAttrs(
      ShPostExchange shPostExchange, File extractFolder, ShPostImpl shPost) {
    for (Entry<String, Object> shPostField : shPostExchange.getFields().entrySet()) {
      ShPostTypeAttr shPostTypeAttr =
          shPostTypeAttrRepository.findByShPostTypeAndName(
              shPost.getShPostType(), shPostField.getKey());
  ...
  private void setTitleAndSummaryFromPost(
      ShPostImpl shPost, Entry<String, Object> shPostField, ShPostTypeAttr shPostTypeAttr) {
    if (shPostTypeAttr.getIsTitle() == (byte) 1) {
      shPost.setTitle(StringUtils.abbreviate((String) shPostField.getValue(), 255));
    } else if (shPostTypeAttr.getIsSummary() == (byte) 1) {
      shPost.setSummary(StringUtils.abbreviate((String) shPostField.getValue(), 255));
  ...
  private void importStaticFiles(
      ShPostExchange shPostExchange,
      File extractFolder,
      ShPostImpl shPost,
      Entry<String, Object> shPostField,
    ...
    if (shPostTypeAttr.getName().equals(ShSystemPostTypeAttr.FILE)
      ...
      File fileSource =
          new File(extractFolder.getAbsolutePath().concat(File.separator + shPostExchange.getId()));
      ...
      try {
        ...
        if (fileSource.getAbsoluteFile().exists()) {
          ...
          if (fileDest.createNewFile()) {
            logger.error(
                "{} file not exists, created empty file into {}.",
                fileSource.getAbsoluteFile(),
                fileDest.getAbsoluteFile());
          } else {
            logger.error(
                "{} file not exists, but it cannot to create empty file into {}.",
                fileSource.getAbsoluteFile(),
                fileDest.getAbsoluteFile());
          }
        }
  ...
  private void createShPostAttrs(
      ShExchangeContext context,
      ShPostExchange shPostExchange,
      ShPost shPost,
      Map<String, Object> shPostFields,
    ...
    for (Entry<String, Object> shPostField : shPostFields.entrySet()) {
      ...
      this.createReferecedPosts(
          context, shExchangeObjectMap, shPostField, shPostType, shPostTypeAttr);
      if (isRelator(shPostTypeAttr)) {

        this.detectPostAttrRelator(
            context,
            shPostExchange,
            shPost,
            shParentRelatorItem,
  ...
  private void createReferecedPosts(
      ShExchangeContext context,
      ShExchangeObjectMap shExchangeObjectMap,
      Entry<String, Object> shPostField,
      ShPostType shPostType,
    ...
    if (isReferencedPostTypeAttr(shPostType, shPostTypeAttr)
        && shPostField != null
  ...
  private boolean isFilePostType(ShPostType shPostType) {
    return shPostType != null
        && shPostType.getName() != null
        && shPostType.getName().equals(ShSystemPostType.FILE);
  }
  ...
  private void detectPostAttrNonRelator(
      ShPost shPost,
      ShRelatorItemImpl shParentRelatorItem,
      Entry<String, Object> shPostField,
      ShPostTypeAttr shPostTypeAttr) {
    ShPostAttr shPostAttr =
        this.importPostAttr(shPost, shParentRelatorItem, shPostField, shPostTypeAttr);
  ...
  @SuppressWarnings({"unchecked"})
  private void detectPostAttrRelator(
      ShExchangeContext context,
      ShPostExchange shPostExchange,
      ShPost shPost,
      ShRelatorItemImpl shParentRelatorItem,
    ...
    ShRelatorItemExchanges subPosts = this.getSubPosts(relatorFields);
    this.createRelatorFromSubPosts(
        context, shPostExchange, shPost, shExchangeObjectMap, shPostAttr, subPosts);
  }

  private void createRelatorFromSubPosts(
      ShExchangeContext context,
      ShPostExchange shPostExchange,
      ShPost shPost,
      ShExchangeObjectMap shExchangeObjectMap,
