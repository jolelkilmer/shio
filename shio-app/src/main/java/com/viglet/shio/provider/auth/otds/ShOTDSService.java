...
public class ShOTDSService {
  ...
  public boolean isAuthorizedUser(String username, String password) {
    ...
    try {
      ...
      HttpPost httpPost =
          new HttpPost(
              String.format(
                  "http://%s:%d/otdsws/rest/authentication/credentials",
      ...
      ShOTDSAuthenticationResponseBean authenticationResponse =
          objectMapper.readValue(responseString, ShOTDSAuthenticationResponseBean.class);
    ...
    if (otdsInstance != null) {
      ...
      try {
        ...
        HttpPost httpPost =
            new HttpPost(
                String.format(
                    "http://%s:%d/otdsws/rest/authentication/resource/validation",
                    otdsInstance.getHost(), otdsInstance.getPort()));
        httpPost.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON.toString());
        httpPost.setHeader(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON.toString());
        httpPost.setEntity(entity);
  ...
  public ShOTDSUserResponseBean getUser(String userName) {
    ...
    HttpGet httpGet =
        new HttpGet(
            String.format(
                "http://%s:%d/otdsws/rest/users/%s",
                otdsInstance.getHost(), otdsInstance.getPort(), userName));
  ...
  public ShOTDSProviderInstanceBean getInstance(String providerId) {
    ShAuthProviderInstanceBean shAuthProviderInstanceBean =
        shAuthProviderService.getShAuthProviderInstanceBean(providerId);
