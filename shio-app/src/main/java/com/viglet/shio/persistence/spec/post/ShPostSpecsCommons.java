...
public class ShPostSpecsCommons {
  ...
  public static List<Predicate> predicateAttrCondition(
      String attrName,
      String attrValue,
      String condition,
      Root<?> root,
    ...
    if (StringUtils.isEmpty(condition) || condition.equals(EQUAL)) {
      ...
      predicates.add(
          criteriaBuilder.and(
              criteriaBuilder.like(root.get(attrName), String.format("%%%s%%", attrValue))));
    } else if (condition.equals(NOT_CONTAINS)) {
      predicates.add(
          criteriaBuilder.and(
              criteriaBuilder.notLike(root.get(attrName), String.format("%%%s%%", attrValue))));
