 ...
 */
@Repository
public interface ShPostDraftRepository
    extends JpaRepository<ShPostDraft, String>, JpaSpecificationExecutor<ShPostDraft> {

  Set<ShPostDraft> findByShPostTypeAndShPostAttrsIn(
  ...
  @Query(
      "select new com.viglet.shio.bean.ShPostTinyBean(p) from ShPostDraft p where p.shFolder.id ="
          + " ?1")
  List<ShPostTinyBean> findByShFolderTiny(String shFolderId);

  @Query(
      "select new com.viglet.shio.bean.ShPostTinyBean(p) from ShPostDraft p where p.shFolder.id ="
          + " ?1 and p.shPostType.id = ?2")
  ...
  List<ShPostDraft> findByShFolderAndShPostTypeOrderByPositionAsc(
      ShFolder shFolder, ShPostType shPostType);

  @Query(
      "select p from ShPostDraft p JOIN FETCH p.shPostType JOIN FETCH p.shFolder JOIN FETCH"
