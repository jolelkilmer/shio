...
public class ShSitesPostUtils {
  ...
  public boolean isFolderIndex(ShPost shPost) {
    if (shPost.getShPostType().getName().equals(ShSystemPostType.FOLDER_INDEX)) {
      if (logger.isDebugEnabled()) logger.debug("Is FolderIndex Post");
      return true;
    } else {
      if (logger.isDebugEnabled()) logger.debug("Is Not FolderIndex Post");
      return false;
    }
  }
  ...
  private ShPost getLivePost(ShPost shPost) {
    ...
    if (logger.isDebugEnabled()) logger.debug("mgmt is not enabled: ");
    if (shPost.isPublished()) {
      if (logger.isDebugEnabled()) logger.debug("is Published ");
      return shPost;
    } else {
      if (logger.isDebugEnabled()) logger.debug("is not Published ");
    }
    return null;
  }

  private ShPost getMgmtPost(ShPost shPost) {
    if (logger.isDebugEnabled()) logger.debug("mgmt is enabled");
    ...
    if (shPostDraftOptional.isPresent()) {
      ...
      if (shPostDraft != null) return shPostDraft;
    }
    return shPost;
  }
  ...
  public List<ShPost> getPostsByStage(List<ShPost> shPosts) {
    ...
    for (ShPost shPost : shPosts) {
      ...
      if (shSelectedPost != null) shSelectedPosts.add(shSelectedPost);
    }
  ...
  public ShPostAttrImpl getPostAttrByStage(ShPostAttrImpl shPostAttr) {
    if (shPostAttr != null) {
      if (shMgmtProperties.isEnabled()) {
        Optional<ShPostDraftAttr> shPostDraftAttrOptional =
            shPostDraftAttrRepository.findById(shPostAttr.getId());
  ...
  private ShPost getPostNested(ShPostAttrImpl shPostAttr) {
    if (shPostAttr.getShParentRelatorItem() != null
      ...
      else return this.getPostNested(shPostAttr.getShParentRelatorItem().getShParentPostAttr());
    }
    return null;
  }
  ...
  public ShContent toSystemMap(ShPostImpl shPostImpl) {
    ...
    for (ShPostAttrImpl shPostAttr : shPostImpl.getShPostAttrs()) {
      if (shPostAttr.getShPostTypeAttr() != null
          && shPostAttr.getShPostTypeAttr().getName() != null) {
  ...
  public List<Map<String, ShPostAttr>> relationToMap(ShPostAttrImpl shPostAttr) {
    ...
    if (shPostAttr != null) {
      ...
      Set<ShRelatorItem> shRelatorItems =
          (Set<ShRelatorItem>) shPostAttr.getShChildrenRelatorItems();
      List<ShRelatorItem> shRelatorItemsByOrdinal = new ArrayList<>();
      shRelatorItemsByOrdinal.addAll(shRelatorItems);

      Collections.sort(
          shRelatorItemsByOrdinal,
      ...
      for (ShRelatorItemImpl shRelatorItem : shRelatorItemsByOrdinal) {
        ...
        for (ShPostAttrImpl shPostAttrRelation : shRelatorItem.getShChildrenPostAttrs()) {
          shRelationMap.put(
              shPostAttrRelation.getShPostTypeAttr().getName(), (ShPostAttr) shPostAttrRelation);
        }
  ...
  public String generatePostLink(ShPostImpl shPostImpl) {
    ...
    if (shPostImpl.getShPostType().getName().equals(ShSystemPostType.FILE)) {
      link =
          shStaticFileUtils.getFileSourceBase(true)
              + "/"
              + shFolderUtils.getSite(shFolder).getName()
              + shFolderUtils.folderPath(shFolder, false, true)
              + shPostImpl.getTitle();
