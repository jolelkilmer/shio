...
public class ShSitesContent {
  ...
  public ShContent fromURL(String url) {
    ...
    if (shObject instanceof ShFolder shFolder) {
      ...
      return isFolderIndex(shPostImpl)
          ? this.fromFolderIndex(shSite, shPostImpl, shSitesContextURL)
  ...
  private ShContent fromFolderIndex(
      ShSite shSite, ShPostImpl shPostImpl, ShSitesContextURL shSitesContextURL) {
    ...
    Map<String, ShPostAttr> shFolderPageLayoutMap =
        shSitesContextComponent.shFolderPageLayoutMapFactory(
            shPostImpl, shSite, shSitesContextURL.getInfo().getShFormat());
  ...
  private ShContent fromPost(
    ...
    try {
      shSitePostTypeLayouts =
          mapper.readValue(
              postTypeLayout.get(shPostImpl.getShPostType().getName()).toString(),
  ...
  private ShContent fromFolder(
      ShSite shSite, ShFolder shFolder, ShSitesContextURL shSitesContextURL) {
    Map<String, Object> shPostItemAttrs = new HashMap<>();
    ShContent shFolderItemAttrs = shSitesFolderUtils.toSystemMap(shFolder);

    Map<String, ShPostAttr> shFolderPageLayoutMap =
        shSitesContextComponent.shFolderPageLayoutMapFactory(
            shFolder, shSite, shSitesContextURL.getInfo().getShFormat());
    if (shFolderPageLayoutMap != null) {
      String shPostThemeId = shFolderPageLayoutMap.get(ShSystemPostTypeAttr.THEME).getStrValue();
