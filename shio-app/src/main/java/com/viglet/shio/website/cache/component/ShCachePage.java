...
public class ShCachePage {
  ...
  @Autowired private ShSitesPage shSitesPage;
  ...
  public ShCachePageBean cache(ShSitesContextURL shSitesContextURL) {
    ...
    String mimeType =
        updatedMimeType(shSitesContextURL, shCachePageBean, shSitesPageLayout, shSite);
  ...
  private String updatedMimeType(
    ...
    if (shObject instanceof ShFolder) {
      if (logger.isDebugEnabled()) logger.debug("Is Folder");
      mimeType = setFolderInfo(shSitesContextURL, mimeType, shSite, shSitesPageLayout);

    } else if (shObject instanceof ShPost) {
      if (logger.isDebugEnabled()) logger.debug("Is Post");
      mimeType =
          setPostInfo(
              shSitesContextURL, shCachePageBean, mimeType, shObject, shSite, shSitesPageLayout);
    }
    return mimeType;
  }

  private String setPostInfo(
      ShSitesContextURL shSitesContextURL,
      ShCachePageBean shCachePageBean,
      String mimeType,
      ShObjectImpl shObject,
  ...
  private String contentTypeByFormat(
      ShSitesContextURL shSitesContextURL,
      String mimeType,
      String format,
    ...
    if (format.equalsIgnoreCase("default")) {
      if (shSitesContextURL.getInfo().getContextURL().endsWith(".json")) mimeType = "json";
    } else {
  ...
  private void setCustomExpirationDate(
      ShCachePageBean shCachePageBean, ShSitesPageLayout shSitesPageLayout) {
  ...
  private void setExpirationDate(
      ShCachePageBean shCachePageBean, Map<String, ShPostAttr> shFolderIndexMap) {
    // TTL
  ...
  private void setBody(
    ...
    try {
      shPageLayoutHTML =
          shSitesContextComponent.shPageLayoutFactory(
              shSitesPageLayout, shSitesContextURL.getRequest(), shSite, mimeType);
    } catch (Exception e) {
  ...
  private void setContentType(ShCachePageBean shCachePageBean, String mimeType) {
    if (mimeType.equals("json")) shCachePageBean.setContentType(MediaType.APPLICATION_JSON_VALUE);
    else if (mimeType.equals("xml"))
      shCachePageBean.setContentType(MediaType.APPLICATION_XML_VALUE);
    else shCachePageBean.setContentType(MediaType.TEXT_HTML_VALUE);
  }
