...
public class ShSitesNodeJS {
  ...
  public StreamingResponseBody exportApplication(
    ...
    if (shSiteOptional.isPresent() && userDir.exists() && userDir.isDirectory()) {
      ...
      File siteFile =
          new File(
              tmpDir
                  .getAbsolutePath()
                  .concat(File.separator + shSite.getName() + UUID.randomUUID() + ".zip"));
      try {

        FileUtils.copyURLToFile(
            new URL("https://github.com/ShioCMS/shio-nodejs/archive/master.zip"), siteFile);
      } catch (Exception e) {
