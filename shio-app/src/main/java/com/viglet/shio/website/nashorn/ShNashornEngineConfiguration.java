...
public class ShNashornEngineConfiguration {
  ...
  private static final String NASHORN_CLASS =
      "org.openjdk.nashorn.api.scripting.NashornScriptEngineFactory";
  private static final String RHINO_CLASS =
      "org.openjdk.nashorn.api.scripting.NashornScriptEngineFactory";
  ...
  public ScriptEngine scriptEngine(ShNashornEngineBindings shBindings) {
    ...
    try {
      ScriptEngine engine =
          defaultEngine.equals(NASHORN_ENGINE) ? nashornEngine(shBindings) : rhinoEngine();
  ...
  private ScriptEngine rhinoEngine()
    ...
    ScriptEngineFactory scriptEngineFactory =
        (ScriptEngineFactory) rhinoScriptEngineFactory.getDeclaredConstructor().newInstance();
    engine = (ScriptEngine) getScriptEngine.invoke(scriptEngineFactory);
    return engine;
  }

  private ScriptEngine nashornEngine(ShNashornEngineBindings shBindings)
      throws ClassNotFoundException, NoSuchMethodException, InstantiationException,
          IllegalAccessException, InvocationTargetException {
    ScriptEngine engine;
    Class<?> nashornScriptEngineFactory = Class.forName(NASHORN_CLASS);
    Method getScriptEngine =
        nashornScriptEngineFactory.getDeclaredMethod("getScriptEngine", String[].class);
    ScriptEngineFactory scriptEngineFactory =
        (ScriptEngineFactory) nashornScriptEngineFactory.getDeclaredConstructor().newInstance();
    engine =
        (ScriptEngine)
            getScriptEngine.invoke(
                scriptEngineFactory, shBindings.getShWebsiteProperties().getNashornAsObject());
    return engine;
  }
}
