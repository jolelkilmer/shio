...
public class ShSitesPage {
  ...
  private String postPage(
    ...
    ShSitePostTypeLayouts shSitePostTypeLayouts =
        this.getPostTypeLayouts(shSite, shPostItem, postTypeLayout);
  ...
  private ShSitePostTypeLayouts getPostTypeLayouts(
      ShSite shSite, ShPost shPostItem, JSONObject postTypeLayout) {
  ...
  private void shFolderIndexPage(
      ShSitesPageLayout shSitesPageLayout,
      ShSite shSite,
      ShSitesContextURL shSitesContextURL,
      ShPost shPostItem) {
  ...
  public void shFolderPage(
      ShSitesPageLayout shSitesPageLayout, ShSite shSite, ShSitesContextURL shSitesContextURL) {
    ShFolder shFolderItem =
        shFolderRepository.findById(shSitesContextURL.getInfo().getObjectId()).orElse(null);
    this.shFolderPage(shSitesPageLayout, shSite, shSitesContextURL, shFolderItem);
  }

  private void shFolderPage(
      ShSitesPageLayout shSitesPageLayout,
      ShSite shSite,
      ShSitesContextURL shSitesContextURL,
      ShFolder shFolderItem) {
    ...
    Map<String, ShPostAttr> shFolderPageLayoutMap =
        shSitesContextComponent.shFolderPageLayoutMapFactory(
            shFolderItem, shSite, shSitesContextURL.getInfo().getShFormat());
    if (shFolderPageLayoutMap != null) {
      String id = shFolderPageLayoutMap.get(ShSystemPostTypeAttr.ID).getStrValue();
      if (logger.isDebugEnabled()) logger.debug("Found Folder PageLayout: " + id);
      ...
      shFolderItemAttrs.put(
          FOLDERS, shSitesContextComponent.shChildFolderItemsFactory(shFolderItem));
      ...
      if (logger.isDebugEnabled()) logger.debug("Not Found Folder PageLayout");
    }
  }
}
