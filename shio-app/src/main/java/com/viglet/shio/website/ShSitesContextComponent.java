...
public class ShSitesContextComponent {
  ...
  @Autowired private ShCacheRegion shCacheRegion;
  @Autowired private ShCachePageLayout shCachePageLayout;
  @Autowired private ShCacheJavascript shCacheJavascript;
  ...
  @Autowired private ShCachePreviewHtml shCachePreviewHtml;
  ...
  public List<String> contentPathFactory(String url) {
    ...
    Collections.addAll(
        contentPath, Arrays.copyOfRange(contexts, contextPathPosition, contexts.length));
    return contentPath;
  }
  ...
  public Map<String, Object> shThemeFactory(String postThemeId) {
    ...
    String cssWithInContextEditing =
        themeCSS != null ? themeCSS.concat(inContextEditingCSS) : inContextEditingCSS;
  ...
  public ShFolder shFolderItemFactory(ShPostImpl shPostItem) {
    ...
    if (shPostItem != null
        && shPostItem.getShPostType().getName().equals(ShSystemPostType.FOLDER_INDEX)) {
  ...
  public Map<String, ShPostAttr> shFolderPageLayoutMapFactory(
      ShObjectImpl shObjectItem, ShSite shSite, String format) {
    return shSitesPostUtils.postToMap(
        shSitesPageLayoutUtils.pageLayoutFromFolderAndFolderIndex(shObjectItem, shSite, format));
  }

  public String shPageLayoutFactory(
  ...
  public Document shRegionFactory(
      ShSitesPageLayout shSitesPageLayout,
      String regionResult,
      ShSite shSite,
      String mimeType,
    ...
    Document doc = null;
    if (shSitesPageLayout.getPageCacheKey().endsWith(".json")
        || mimeType.equals("json")
        || mimeType.equals("xml"))
      doc = Jsoup.parse(regionResult, StringUtils.EMPTY, Parser.xmlParser());
    else doc = Jsoup.parse(regionResult);
    ...
    for (Element element : doc.getElementsByAttribute("sh-region")) {
      ...
      if (shCacheRegion.isCached(regionName, shSite.getId()))
        cachedRegion =
            shCacheRegion.templateScopeCache(
                regionName, shSitesPageLayout, shSite, shObjectJS, mimeType, request);
      else
        cachedRegion = this.regionProcess(regionName, shSitesPageLayout, shSite, mimeType, request);

      if (cachedRegion != null) element.html(cachedRegion).unwrap();
      else {
  ...
  public String regionProcess(
    ...
    if (shRegion != null) {
      ...
      Object regionResultChild =
          shNashornEngineProcess.render(
              regionName, shRegionJS, shRegionHTML, request, shSitesPageLayout.getShContent());

      String regionHTML =
          this.shRegionFactory(
                  shSitesPageLayout, regionResultChild.toString(), shSite, mimeType, request)
              .html();
      ...
      Comment comment =
          new Comment(
              String.format(
                  " sh-region: %s, id: %s, processed in: %s ms ",
                  regionName, shRegion.getId(), String.valueOf(timeProcess)));
      String previewRegion = shCachePreviewHtml.shPreviewRegionFactory();

      return previewRegion.replace(
          "{{content}}", String.format("%s%s", comment.toString(), regionHTML));
    }
    return null;
  }
  ...
  public ShPost getRegion(String regionName, String siteId) {
    ...
    if (shRegionPosts != null) {
      for (ShPost shRegionPost : shRegionPosts) {
        if (shPostUtils.getSite(shRegionPost).getId().equals(siteId)) shRegion = shRegionPost;
      }
    }
    return shRegion;
  }
