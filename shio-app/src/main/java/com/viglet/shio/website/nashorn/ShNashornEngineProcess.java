...
public class ShNashornEngineProcess {
  ...
  private static final ThreadLocal<HashMap<String, ScriptContext>> scriptContexts =
      new ThreadLocal<>();
  ...
  public Object render(
    ...
    try {
      ...
      if (b != null) {
        ...
        if (logger.isDebugEnabled()) logger.debug("Bindings is null");
      }
      ...
      if (logger.isDebugEnabled()) logger.debug("render: " + render);
      return render;
  ...
  public void regionError(String regionAttr, String javascript, Throwable err) {
    if (err instanceof ScriptException exc) {
      ...
      if (lineNumber + 5 < maxlines) maxlines = lineNumber + 5;
      for (int x = minlines; x <= maxlines; x++) {
        errorCode.append(javascriptLines[x] + "\n");
        if (x == lineNumber - 1) {
          String errorPos =
              IntStream.range(0, columnNumber)
                      .mapToObj(i -> "-")
      ...
      logger.error(
          String.format(
              "ScriptError: %s '%s' at: <%s>%d:%d%n%s",
              regionAttr, message, fileName, lineNumber, columnNumber, scriptStack));
    } else {
